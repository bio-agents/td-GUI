...
Ext.define('Ext.Neptune.panel.Agent', {
	override: 'Ext.panel.Agent',
	
    renderTpl: ['<div id="{id}-agentEl" class="{baseCls}-{type}" role="presentation"></div>']
});
...
Ext.define('Ext.Neptune.window.MessageBox', {
	...
	initComponent: function() {
        ...
        me.bottomTb = new Ext.agentbar.Agentbar({
            ui: 'footer',
            dock: 'bottom',
            layout: {
                pack: 'end'
    ...
    beforeRenderItems: function (items) {
        ...
        for (i = 0; i < ln; i++) {
            ...
            if (!comp.rendered) {
                ...
                if (me.hideCollapseAgent) {
                    comp.hideCollapseAgent = me.hideCollapseAgent;
...
Ext.define('Ext.Neptune.panel.Header', {
    ...
    initComponent: function() {
        ...
        if (me.agentsFirst) {
            me.addCls(me.baseCls + '-agents-first');
            me.items = me.items.concat(me.agents);
...
Ext.define('Ext.Neptune.panel.Panel', {
    ...
    /**
     * True to makes the agents of this panels header go to the left of the panel header title. Used for accordions
     * @cfg {Boolean} agentsFirst
     * @private
     */
    agentsFirst: false,
    ...
    updateHeader: function(force) {
        ...
        if (!me.preventHeader && (force || title || (agents && agents.length))) {
            if (header) {
                header.show();
            } else {
                ...
                // Reference the Header's agent array.
                // Header injects named references.
                me.agents = header.agents;
            }
            me.initHeaderAria();
...
Ext.define('Ext.Neptune.resizer.Splitter', {
    ...
    onRender: function() {
        ...
        // Add listeners on the mini-collapse agent unless performCollapse is set to false
